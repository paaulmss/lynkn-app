  ...
  return (
    <div className="confirm-modal-overlay" onClick={onClose}>
      ...
      >
        ...
        <p>
          Estás a punto de borrar <strong>"{title}"</strong>. Esta acción es
          irreversible y eliminará todos los datos asociados.
        </p>
...
const PostDetailModal: React.FC<PostDetailModalProps> = ({ post, onClose }) => {
  ...
  const percentage = isUnlimited
    ? 0
    : Math.min(100, Math.floor((current / (max || 1)) * 100));
    ...
    const fetchParticipants = async () => {
      try {
        ...
        const myRequest = response.data.find(
          (p: Participant) => Number(p.user_id) === Number(user?.id),
        );
  ...
  const handleParticipantAction = async (
    ...
    try {
      ...
      const wasAccepted =
        participants.find((p) => p.id === id)?.status === "accepted";
            ...
            {isOwner ? (
              ...
              <div className="nomad-user-controls">
                <div className="nomad-footer-status">
                  <span
                    className="nomad-spots-left"
                    style={{ color: percentage > 90 ? "#ef4444" : "#71717a" }}
                  >
                    {isUnlimited
                      ? "EVENTO ILIMITADO"
                  ...
                  {!isUnlimited && (
                    <div className="nomad-bar-container">
                      <div
                        ...
                        style={{
                          width: `${percentage}%`,
                          backgroundColor: percentage > 80 ? "#ef4444" : "#fff",
                        }}
                      ></div>
                    </div>
                  )}
                ...
                {joinStatus === "rejected" ? (
                  <button
                    className="nomad-btn-status-redirect rejected"
                    disabled
                  >
